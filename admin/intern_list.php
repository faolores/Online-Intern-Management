		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM intern_list order by concat(firstname,' ',lastname) asc");
...
<script>
	...
	$('.view_faculty').click(function(){
		uni_modal("<i class='fa fa-id-card'></i> Intern Details","<?php echo $_SESSION['login_view_folder'] ?>view_faculty.php?id="+$(this).attr('data-id'))
	})
	$('.delete_faculty').click(function(){
	_conf("Are you sure to delete this intern?","delete_faculty",[$(this).attr('data-id')])
	})
		$('#list').dataTable()
	})
