...
Class Action {
	...
	function login(){
		extract($_POST);
		$type = array("","users","intern_list","industry_professor_list");
		$type2 = array("","admin","intern","industry_professor");
	...
	function save_faculty(){
		...
		$check = $this->db->query("SELECT * FROM intern_list where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		$check = $this->db->query("SELECT * FROM intern_list where school_id ='$school_id' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 3;
			exit;
		}
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO intern_list set $data");
		}else{
			$save = $this->db->query("UPDATE intern_list set $data where id = $id");
		}
	...
	function delete_faculty(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM intern_list where id = ".$id);
		if($delete)
			return 1;
	}
	function save_student(){
		...
		$check = $this->db->query("SELECT * FROM industry_professor_list where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO industry_professor_list set $data");
		}else{
			$save = $this->db->query("UPDATE industry_professor_list set $data where id = $id");
		}
	...
	function delete_student(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM industry_professor_list where id = ".$id);
		if($delete)
			return 1;
	}
	function save_task(){
	...
	function get_class(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT c.id,concat(c.curriculum,' ',c.level,' - ',c.section) as class,s.id as sid,concat(s.code,' - ',s.subject) as subj FROM restriction_list r inner join class_list c on c.id = r.class_id inner join subject_list s on s.id = r.subject_id where r.intern_id = {$fid} and academic_id = {$_SESSION['academic']['id']} ");
